...
class API {
	...
	private function put_testdata($TESTID,$PROFILEID,$data){
		...
		// synesthesia sweden: send results to db during test multiple times
		
		// test if profileid exists

		$query = "SELECT COUNT(*) FROM $TESTID WHERE PROFILEID = '".$PROFILEID."'";
		try {
			// error_log($query);
		...
		if($count == 0){
			...
			$q['PROFILEID'] = $PROFILEID;
			$q['IP'] = get_ip_address();
			$query = "UPDATE $TESTID ".$this->make_update_query($q)." WHERE PROFILEID = '".$PROFILEID."'";
			try {
				// error_log($query);
				$stmt = $this->data_obj->dbc->query($query);
	...
	private function make_update_query($array){
