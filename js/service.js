app.service("gno", function($localStorage,$http,$q,$timeout) {
    ...
    this.get_random_set_questions = function(testid){
        ...
        if(check) { // return same questions if already started
            ...
            $http.get("api/questions/"+testid).success(function(data){
    ...
    this.put = function(testid){
        ...
        $timeout(function(){
            $http.post("api/put/"+testid+"/"+self.$storage.profile.PROFILEID,data).success(function(data){
    ...
    this.score = function(testid){
        ...
        if(testinfo[testid].type=="syn" || testinfo[testid].type=="synplus"){
            ...
            $.each(spread,function(k,v){
                ...
                var distance = d ? (3-d[3]) * (100/3) : false;
                if(distance){
                    v.distance = parseInt(distance);
                    calc.push(distance);
                    total = total+distance;
                }
                if(testid=="kleurenalfabet" || testid=="lettersencijfers"){
    ...
    this.distance = function(colors){
        if(colors[0] && colors[1] && colors[0]!='nocolor' && colors[1]!='nocolor'){
            ...
            var r = Math.abs(c1[0]-c2[0]);
            var g = Math.abs(c1[1]-c2[1]);
            var b = Math.abs(c1[2]-c2[2]);
            return [r,g,b,(r+g+b)];
        } else {
            return false;
        }
