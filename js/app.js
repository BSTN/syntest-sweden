...
app.config(function($stateProvider, $urlRouterProvider, $locationProvider) {
    ...
    $stateProvider
        ...
        .state('info', {
            url: '/info',
            template: "<div koppen id='head'></div><div class='text info' compile='html' ng-class='name'></div><div bstn></div>",
            controller: 'staticCtrl'
        })
        .state('test', {
...
app.run(function($rootScope, $http, $interval, $location, $state, $localStorage, $timeout, $window, gno, popup) {
    ...
    $rootScope.reset = function() {
        // popup.confirm("Är du säker på att du vill börja från början? <label>Profilen och testresultaten i din webbläsare kommer att raderas. Använd den här knappen endast för att starta ett nytt test.</label>",["ja","nej"]).then(function(sure){
        //     if(sure=="ja"){
...
app.controller("testCtrl", function($scope, $rootScope, $state, $localStorage,$templateCache, $timeout, $http, gno, popup) {
    ...
    $scope.questionsfinished = function(){
        if($scope.$storage.profile.Q8 || $scope.$storage.profile.NOTHING){
            return true;
        } else {
            return false;
        }
    ...
    $scope.checkaltprofiel = function(){
        if($scope.$storage.profile.CODE && $scope.$storage.profile.GEBOORTEDATUM){
            if($scope.$storage.profile.CODE!="" && $scope.$storage.profile.GEBOORTEDATUM!=""){
                return true;
            }
        }
