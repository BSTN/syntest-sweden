...
app.directive("toresults",function($state,gno){
    return {
        restrict: "A",
        link: function(s,el,a){
            el.on("click",function(){
                gno.put("profile");
                $state.go("results",{"testid":$state.params.testid});
            })
        }
    }
})
...
app.directive("koppen",function($rootScope,$timeout){
    return {
        restrict:"A",
        template:"<div id='in' ng-class=\"{'open':$root.aboutmenu}\"><a href='tests' id='logo'></a><div id='burger' ng-click=\"$root.aboutmenu=!$root.aboutmenu\"><i></i></div><div id='omkop'><div id='kop' ng-repeat='(k,kop) in koppen' ng-click='skrolto(k)' ng-class=\"{'active':k==n}\">{{kop}}</div></div><div id='line'></div></div>",
        link:function(s,el,a){
            s.n = 0;
...
app.directive("bstn",function($timeout){
    return {
        restrict:"A",
        template: "deze website is ontwikkeld en ontworpen door:<br><a href='http://bstn.nl' target='_blank'>BSTN - Brother, Salute The Namedrop!<a/>",
        link:function(s,el,a){
