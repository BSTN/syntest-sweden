app.directive("one2five", function($localStorage){
    return {
        restrict:"A",
        template:"<button ng-click='set(1)' ng-class='{\"active\":temp==1}'>1 <label>Stämmer inte alls</label></button><button ng-click='set(2)' ng-class='{\"active\":temp==2}'>2</button><button ng-click='set(3)' ng-class='{\"active\":temp==3}'>3</button><button ng-click='set(4)' ng-class='{\"active\":temp==4}'>4</button><button ng-click='set(5)' ng-class='{\"active\":temp==5}'>5 <label>Stämmer fullständigt</label></button>",
        scope:{},
        link:function(s,el,a){
            s.temp = $localStorage.profile['Q'+a.one2five];
            s.set = function(val){
                s.temp = val;
                $localStorage.profile['Q'+a.one2five] = val;
            }
        }
    }
})
